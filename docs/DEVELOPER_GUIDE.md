# Developer Guide: Sovereign Venture Underwriting
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/venture-underwriting.git
...
export SBB_CRUNCHBASE_API_KEY=your_value_here
export SBB_VALUATION_MODEL=your_value_here
export SBB_RISK_THRESHOLD=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-venture-underwriting)
...
```bash
# Build
docker build -t sovereign-venture-underwriting:dev .

# Run with env file
docker run --env-file .env -p 8795:8795 sovereign-venture-underwriting:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-venture-underwriting
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8795`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-venture-underwriting",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8795` in use | `lsof -i :8795` | Kill PID or change `SERVICE_PORT` |
